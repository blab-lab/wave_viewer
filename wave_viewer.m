...
% no change in horiz_orig, since "pitch params" is in-line with "help"
    function my_help_func(hObject,eventdata)   % callback for help button
        helpstr = sprintf(['Keyboard shortcuts:\n', ...
            ...
            'Right arrow - advance tmarker_spec by one frame\n', ...
            'Left arrow- retreat tmarker_spec by one frame\n', ...
            'H - heighten\n', ...
            'U - unheighten = reduce\n']);
        helpstr_new = strsplit(helpstr, '\n'); % prevents a bug where newlines are added
        
        % Create help dialog
        help_fig = figure('Name', 'Keyboard Shortcuts', 'NumberTitle', 'off', ...
            'MenuBar', 'none', 'ToolBar', 'none', 'Resize', 'off', ...
            'Position', [300, 300, 700, 400], 'Color', 'white');
        
        % Create text control for help dialog
        helpDialogFontSize = .05;
        uicontrol('Style', 'text', 'String', helpstr_new, ...
            'Units', 'normalized', 'Position', [0.05, 0.15, 0.9, 0.8], ...
            'FontUnits', 'normalized', 'FontSize', helpDialogFontSize, ...
            'HorizontalAlignment', 'left', 'BackgroundColor', 'white');
        
        % Add OK button for help dialog
        uicontrol('Style', 'pushbutton', 'String', 'OK', ...
            'Units', 'normalized', 'Position', [0.4, 0.05, 0.2, 0.08], ...
            'FontUnits', 'normalized', 'FontSize', 0.65, 'BackGroundColor', [0.8, 0.8, 0.8], ...
            'Callback', @(src,evt) close(help_fig));
    end
...
hbutton.pitch_params = uicontrol(p.guidata.buttonPanel,'Style','pushbutton',...
    ...
    'FontUnits','Normalized','FontSize',buttonFontSize*0.9,...
...
horiz_orig = horiz_orig + buttonHeight + padYButton;
    function edit_pitch_params(hObject,eventdata) % callback for h_button.pitch_params
        ...
        default_params = get_sigproc_params();
        dlgtitle = 'Params';
        prompt = {strcat('pitch track method (default:  ', default_params.ptrack_method, ')'),...
            sprintf('pitch limit (min) (default: %.0f)', default_params.pitchlimits(1)),...
            sprintf('pitch limit (max) (default: %.0f)', default_params.pitchlimits(2)),...
            sprintf('max candidates (default: %.0f)', default_params.ptrack_max_candidates),...
            sprintf('silence threshold (default: %1.2f)', default_params.ptrack_silence_thresh),...
            sprintf('voicing threshold (default: %1.2f)', default_params.ptrack_voicing_thresh),...
            sprintf('octave cost (default: %1.2f)', default_params.ptrack_octave_cost),...
            sprintf('octave jump cost (default: %1.2f)', default_params.ptrack_octave_jump_cost),...
            sprintf('voiced/unvoiced cost (default: %1.2f)', default_params.ptrack_voiced_unvoiced_cost),...
            };
...
% previous button
prevButtonPos = [padL horiz_orig buttonWidth/2 buttonHeight];
...
% continue button
contButtonPos = [padL+buttonWidth*1/2 horiz_orig buttonWidth/2 buttonHeight];
hbutton.cont = uicontrol(p.guidata.buttonPanel,'Style','pushbutton',...
    'String','continue',...
...
hbutton.cont = uicontrol(p.guidata.buttonPanel,'Style','pushbutton',...
    'String','<html><center><hbutton style = "line-height: 5px;">rename events</center></html>',...
    'Units','Normalized','Position',editEventsButtonPos,...
    'FontUnits','Normalized','FontSize',buttonFontSize*0.85,...
...
hbutton.clear_events = uicontrol(p.guidata.buttonPanel,'Style','pushbutton',...
    'String','<html><center>clear events</center></html>',...
...
hedit.preemph = uicontrol(p.guidata.buttonPanel,'Style','edit',...
    ...
    'FontUnits','Normalized','FontSize',editFontSize*0.8,...
...
horiz_orig = horiz_orig + tinyHeight + padYBig*0.75;
...
hedit.ampl_thresh4voicing = uicontrol(p.guidata.buttonPanel,'Style','edit',...
    ...
    'FontUnits','Normalized','FontSize',editFontSize*0.7,...
